...
ExecStart=/var/antctl/services/$node_name/antnode --no-upnp --bootstrap "164.92.111.156:12000" --root-dir /var/antctl/services/$node_name --port $(("$ntpr"000 + $NextNodeToSorA)) --enable-metrics-server --metrics-server-port $((13000 + $NextNodeToSorA)) --log-output-dest /var/log/antnode/$node_name --max-log-files 1 --max-archived-log-files 1 $RewardsAddress evm-arbitrum-one
Restart=always
#RestartSec=300
EOF

#--bootstrap-cache-dir /var/antctl/bootstrap-cache
