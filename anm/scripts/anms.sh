...
[Service]
User=safe
ExecStart=/var/safenode-manager/services/safenode$NextNodeToSorA/safenode --root-dir /var/safenode-manager/services/safenode$NextNodeToSorA --port $ntpr$node_number --enable-metrics-server --metrics-server-port 13$node_number $DiscordUsername
Restart=on-failure
EOF
    #  --log-output-dest /var/log/safenode/safenode$NextNodeToSorA --max_log_files 5 --max_archived_log_files 5
...
RemoveNode() {
    ...
    sudo rm -rf /var/safenode-manager/services/safenode$1 /var/log/safenode/safenode$1
    echo "rm -rf /var/safenode-manager/services/safenode$1 /var/log/safenode/safenode$1"
...
StoppedUpgrade() {
    ...
    echo "cp $NodePath /var/safenode-manager/services/safenode$1"
    PIS=$(echo "${node_details_store[$node_number]}" | awk -F',' '{print $2}')
    node_details_store[$node_number]="$node_name,$PIS,$(/var/safenode-manager/services/safenode$1/safenode -V | awk '{print $3}'),STOPPED"
