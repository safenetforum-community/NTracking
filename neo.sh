...
for (( i = 1; i <= $NumberOfNodes; i++ )); do
    ...
    node_name=safenode$node_number
        ...
        # Format for InfluxDB
        node_details_str[$i]="nodes,id=$node_name status=$status,records="$records"i,connected_peers="$connected_peers"i,rewards=$rewards_balance,store_cost="$store_cost"i,cpu="$cpu_usage"i,mem="$mem_used"i,puts="$puts"i,gets="$gets"i,version=$NodeVersion,networ_size="$network_size"i $influx_time"
