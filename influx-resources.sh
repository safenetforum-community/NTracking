...
for ((i = 1; i <= $NumberOfNodes; i++)); do
    ...
    if [[ -n "$node_details" ]]; then
        ...
        if [[ -f "/var/safenode-manager/NodeDetails" ]]; then
            ...
            status="$(sudo systemctl status safenode$i.service --no-page)"
    ...
    # Format for InfluxDB
    node_details_str[$i]="nodes,id=$node_name,$PeerId status=$status,records="$records"i,connected_peers="$connected_peers"i,rewards=$rewards_balance,store_cost="$store_cost"i,cpu="$cpu_usage"i,mem="$mem_used"i,puts="$puts"i,gets="$gets"i,version=$NodeVersion $influx_time"
